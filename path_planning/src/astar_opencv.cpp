...
//*** To Visualize in runtime for path funding
  ...
  for (auto dir : directions) {
      ...
      // Check if the new position is valid, and not already in the closed list
    if (new_x >= 0 && new_x < map.rows && new_y >= 0 && new_y < map.cols && !closed_list[new_x][new_y] ) {
...
int main() {
  ...
  cv::imshow("Astar All Possible Paths", map);
  cv::waitKey(0);
