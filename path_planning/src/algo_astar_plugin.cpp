...
bool compare_node::operator()(std::shared_ptr<NodeAstar> const& a, std::shared_ptr<NodeAstar> const& b) const {
...
std::vector<geometry_msgs::msg::PoseStamped> astar_search(nav_msgs::msg::OccupancyGrid const& grid,
	...
	std::cout << "Free cells: " << free_cells << ", Occupied cells: " << occupied_cells << std::endl;
	...
	start_point.x = static_cast<float>(start_x);
	start_point.y = static_cast<float>(start_y);

	goal_point.x = static_cast<float>(goal_x);
	goal_point.y = static_cast<float>(goal_y);

	// Print start and goal points
	std::cout << "Starting point: (" << start_point.x << ", " << start_point.y << ") - Goal point: (" << goal_point.x
	          << ", " << goal_point.y << ")" << std::endl;
	std::cout << "Map Width: " << GRID_WIDTH << ", Map Height: " << GRID_HEGHT << std::endl;

	std::priority_queue<std::shared_ptr<NodeAstar>, std::vector<std::shared_ptr<NodeAstar>>, compare_node> open_list;
	std::vector<std::vector<float>> cost_so_far(GRID_HEGHT,
	...
	start_node->set_hcost(
	    heuristic(start_node->x, start_node->y, static_cast<int>(goal_point.x), static_cast<int>(goal_point.y)));
	...
	while (!open_list.empty()) {
		...
		std::cout << "Checking node at (" << current_node->x << ", " << current_node->y << ") against goal ("
		          << goal_point.x << ", " << goal_point.y << ")" << std::endl;
		...
		if (current_node->x == goal_point.x && current_node->y == goal_point.y) {
			...
			std::cout << "Found goal path. Total number of nodes in the path: " << node_count << std::endl;
